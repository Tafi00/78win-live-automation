...
class LiveTracker {
  ...
  async startTracking(liveUrl, username, config = {}) {
    ...
    try {
      ...
      // Khởi động Turnstile Token Farmer nạp token trước vào bể
      this.farmer = new TurnstileFarmer({
        url: liveUrl,
        onStatus: (msg) => this.log('FARMER', msg)
      });
      await this.farmer.start();
      if (this.config.autoLixi) {
        this.farmer.setDemand(true, Math.min(3, this.targetAccounts.length));
      }
  ...
  startAttendanceScheduler() {
    ...
    this.attendanceTimer = setInterval(async () => {
      try {
        ...
        const offsets = (info.attendanceConfig && info.attendanceConfig.offsetsMinutes) || [];
        ...
        for (const acc of this.targetAccounts) {
          ...
          // Đánh dấu ngay để tránh trùng lặp trong window
          ...
          fetch(`${API_BASE}/livestreams/${streamId}/attendance`, {
            ...
            if (res && (res.error === 0 || res.data)) {
              ...
              // "Ngoài khung giờ" thường do lệch đồng hồ giây đầu window hoặc startedAt stale
              // -> cho retry tối đa 8 lần trong window
            ...
            this.attendanceDone.delete(key); // lỗi mạng -> cho retry tick kế tiếp
            this.log('ERROR:ATTENDANCE', `[${acc.username}] Lỗi kết nối: ${err.message}`);
          });
        }
      } catch (e) {
        // Scheduler không bao giờ chết vì 1 tick lỗi
      }
    }, 2000);
  }
  ...
  handleBrowserEvent(evt) {
    ...
    switch (evt.type) {
      ...
      case 'LIXI_CLICKED':
        ...
        if (this.config.autoLixi && this.activeRewardSession && this.activeRewardSession.sessionId) {
          this.claimLixiForAllAccounts(this.activeRewardSession.sessionId, this.activeRewardSession.group);
        }
        break;

      case 'LIXI_MODAL_READY':
        this.log('MODAL', `Popup lì xì đã mở. Đang chuẩn bị nhận lì xì cho tất cả tài khoản...`);
        if (this.config.autoLixi && this.activeRewardSession && this.activeRewardSession.sessionId) {
          this.claimLixiForAllAccounts(this.activeRewardSession.sessionId, this.activeRewardSession.group);
        }
        break;
  ...
  async claimLixiForAllAccounts(sessionId, group) {
    ...
    // Xử lý nhận Lì Xì song song cho TẤT CẢ các tài khoản
    const claimPromises = this.targetAccounts.map(async (acc) => {
      const sess = getSession(acc.username);
      if (!sess || !sess.token) {
        this.log('WARN:LIXI', `[${acc.username}] Bỏ qua nhận lì xì vì chưa có phiên đăng nhập.`);
        return;
      }

      const deviceHash = crypto.createHash('md5').update(`78win-${acc.username}`).digest('hex');

      // 1. Lấy token Turnstile từ bể Token Farmer hoặc chờ nạp mới
      let turnstileToken = null;
      if (this.farmer) {
        turnstileToken = await this.farmer.waitForToken(15000);
      }

      // 2. Fallback: Lấy token từ page nếu trang đã giải xong
      if (!turnstileToken && this.page) {
        try {
          turnstileToken = await this.page.evaluate(() => {
      ...
      if (!turnstileToken) {
        this.log('ERROR:LIXI', `[${acc.username}] Không lấy được token Cloudflare Turnstile.`);
        return;
      }
      ...
      try {
        ...
        if (res && res.error === 0) {
          const rewardPoint = (res.data && res.data.rewardPoint) || 0;
          ...
          const msg = (res && res.message) || 'Thất bại';
          this.log('ERROR:LIXI', `[${acc.username}] Nhận lì xì: ${msg}`);
        }
        this.emit('status', this.getStatus());
      } catch (err) {
        this.log('ERROR:LIXI', `[${acc.username}] Lỗi kết nối claim lì xì: ${err.message}`);
      }
    });

    Promise.allSettled(claimPromises).then(() => {
      this.log('LIXI:SUMMARY', `Hoàn tất xử lý nhận Lì Xì phiên ${sessionId}.`);
      this.emit('status', this.getStatus());
    });

    // Đồng thời thử kích hoạt trên giao diện tab trình duyệt chính
    if (this.page) {
      try {
        const token = this.farmer ? this.farmer.consumeToken() : null;
        if (token) {
    ...
    await this.page.evaluate((cfg) => {
      ...
      setInterval(() => {
        ...
        if (cfg.autoLixi) {
          ...
          if (openBtn) {
            // Kiểm tra token Turnstile trong DOM
            try {
              const el = document.querySelector('[name="cf-turnstile-response"]');
              const v = (el && el.value) || (window.turnstile && typeof window.turnstile.getResponse === 'function' ? window.turnstile.getResponse() : '');
              if (v) {
                window.__emitTrackerEvent({ type: 'TURNSTILE_TOKEN', data: { token: v } });
              }
            } catch (e) {}
            ...
            if (!openBtn.disabled) {
              ...
              if (now - lastClaimClick > 3000) {
                ...
                if (!openBtn.hasAttribute('data-modal-logged')) {
                  ...
                  window.__emitTrackerEvent({ type: 'LIXI_MODAL_READY', data: window.__lastRewardSession || {} });
                }
                openBtn.click();
              }
            }
            // Modal đang mở -> không click envelope
          } else {
            ...
            if (lixiBtns.length > 0) {
              ...
              if (now - lastLixiClick > 4000) {
                lastLixiClick = now;
                window.__emitTrackerEvent({ type: 'LIXI_CLICKED', data: window.__lastRewardSession || {} });
                lixiBtns[0].click();
              }
            }
          }
