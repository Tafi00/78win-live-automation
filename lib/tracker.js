...
class LiveTracker {
  ...
  handleBrowserEvent(evt) {
    ...
    switch (evt.type) {
      ...
      case 'LIXI_ANNOUNCED':
        this.log('WS:LIXI', `🧧 PHONG BAO LÌ XÌ XUẤT HIỆN! Phiên ID: ${evt.data.sessionId}`);
        break;
      ...
      case 'LIXI_MODAL_READY':
        this.log('MODAL', `Popup lì xì đã mở. Đang chờ giải Turnstile Captcha...`);
        break;
        ...
        if (cfg.autoLixi) {
          ...
          if (openBtn) {
            if (!openBtn.disabled) {
              ...
              if (now - lastClaimClick > 3000) {
                ...
                if (!openBtn.hasAttribute('data-modal-logged')) {
                  ...
                  window.__emitTrackerEvent({ type: 'LIXI_MODAL_READY', data: {} });
                }
                openBtn.click();
              }
            }
            ...
            if (lixiBtns.length > 0) {
              const now = Date.now();
              if (now - lastLixiClick > 5000) {
                lastLixiClick = now;
                window.__emitTrackerEvent({ type: 'LIXI_CLICKED', data: {} });
                lixiBtns[0].click();
              }
            }
          }
