...
class LiveTracker {
      ...
      case 'LIXI_CLAIM_HTTP':
        {
          ...
          if (status === 200 && body && body.error === 0) {
            ...
            const alreadyLogged = this.stats.lixiClaimed.some(c =>
              c.sessionId === this.activeRewardSession?.sessionId && c.username === this.primaryAccount
            );
            if (!alreadyLogged) {
              this.stats.lixiClaimed.push({
                username: this.primaryAccount,
                rewardPoint,
                sessionId: this.activeRewardSession?.sessionId,
                at: new Date().toISOString()
              });
  ...
  async claimLixiForAllAccounts(sessionId, group) {
    ...
    // 2. Lấy visitorId chuẩn từ trang trình duyệt (FingerprintJS v4 / J5e fallback)
    let browserVisitorId = null;
    if (this.page) {
      try {
        browserVisitorId = await this.page.evaluate(async () => {
          if (window.__cachedVisitorId) return window.__cachedVisitorId;

          // A. FingerprintJS v4 (chuẩn 100% như frontend của 78win-live)
          try {
          ...
          // B. Fallback đúng chuẩn J5e của frontend web (RewardClaim_AntiBot_2026_v1)
          try {
    ...
    // Helper tạo deviceHash chuẩn định dạng 32-char hex (chuẩn FingerprintJS x64hash128)
    const getDeviceHash = (username) => {
      // Primary account dùng trực tiếp visitorId nếu là 32 ký tự hex
      if (username === this.primaryAccount && browserVisitorId && /^[0-9a-f]{32}$/i.test(browserVisitorId)) {
        return browserVisitorId;
      }
      // Các account khác hoặc fallback: tạo 32-char hex hash độc nhất & đồng nhất cho từng user
      const seed = `${browserVisitorId || '78win_live_v1'}:${username}`;
      return crypto.createHash('md5').update(seed).digest('hex');
    };

    // 3. Xử lý nhận Lì Xì cho TẤT CẢ các tài khoản
    (async () => {
      for (const acc of this.targetAccounts) {
        // Tránh claim trùng nếu giao diện web đã tự mở thành công cho primary account
        const alreadyClaimed = this.stats.lixiClaimed.some(c =>
          c.username === acc.username && c.sessionId === sessionId
        );
        if (alreadyClaimed) {
          continue;
        }
        ...
        const deviceHash = getDeviceHash(acc.username);
        ...
        try {
          ...
          if (this.page) {
            claimResult = await this.page.evaluate(async ({ url, token, turnstileToken, deviceHash }) => {
              try {
                // Dùng __origFetch để tránh bị fetch wrapper chặn lại và gây duplicate log
